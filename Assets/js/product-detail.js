...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let retries = 5;
  let product_loaded = null;

  // Retry loading product with exponential backoff (up to 5 attempts)
  while (retries > 0 && !product_loaded) {
    try {
      // Check if store.js functions are available
      if (typeof getProductBySlug === 'function' && typeof loadProducts === 'function') {
        product_loaded = await getProductBySlug(slug);
        
        // If not found by slug, try to load all and search
        if (!product_loaded && typeof loadProducts === 'function') {
          const all = await loadProducts();
          product_loaded = (all || []).find(p => p.slug === slug || p.id === slug || p.name === slug) || null;
        }
        break;
      }
    } catch (err) {
      console.error(`Error loading product (attempt ${6 - retries}):`, err);
    }
